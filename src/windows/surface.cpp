...
bool mySurface::Create(int width, int height, int depth, bool flip, COLORREF* palette, int palettesize)
{
...
bool mySurface::CreateBitmap(int width, int height, int depth, float dpix, float dpiy, bool flip, COLORREF* palette, int palettesize)
{
	if (Bitmap != NULL)
		Destroy();
...
bool mySurface::IsValid()
{
	return ((SurfaceDC != nullptr) && (Bitmap != nullptr));
}

void mySurface::SetDPI(float dpix, float dpiy)
{
...
void mySurface::Clear(myColor c)
{
	if (!IsValid()) return;		
	if (Depth == 32)
	{		
		myColor* p = (myColor*)GetLinePointer(0);
...
mySurface* LoadSurface(Gdiplus::Bitmap& image)
{
...
mySurface* ChangeDepthSurface(mySurface* surface,int depth)
{
	...
	mySurface* newsurface = new mySurface();
	if (newsurface == NULL)
	{
...
mySurface* LoadSurface(std::wstring path)
{
	...
	mySurface* surface = LoadSurface(image);
	if (surface == nullptr) return nullptr;
	return surface;
}
