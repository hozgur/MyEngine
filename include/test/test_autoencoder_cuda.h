...
public:
	...
	void run()
	{
		...
		tensor<float>* tensIn = (tensor<float>*) GetMyObject(id);
		tensor<float>* tensOut = (tensor<float>*) GetMyObject(ido);
		...
		if (tensIn != nullptr)
		{
			for (int i = 0; i < 2; i++)
			{
				for (int j = 0; j < 8; j++)
				{
					...
					{
						float* tData = tensIn->getData(1, y + (i * 8 + j) * height);
					...
					for (int y = 0; y < height; y++)
					{
						float* tData = tensOut->getData(1, y + (i * 8 + j) * height);
