...
namespace myPy
{
	...
	static int pytensor_init(pytensor* self, PyObject* args) {
		...
		if (tensor == nullptr)
			return -1;
		myEngine::pEngine->removeObject(self->tensorId);
		self->tensorId = myEngine::pEngine->setObject((myObject*)tensor);
		self->type[0] = p.first;
		self->type[1] = 0;
		return 0;
	}
	...
	static PyObject* pytensor_str(pytensor* self) {
		...
		buffer << *(myTensor<uint8_t>*)myEngine::pEngine->getObject(self->tensorId);
	...
	static int pytensor_getbuffer(PyObject* obj, Py_buffer* view, int flags)
	{
		...
		myTensor<uint8_t>* t = (myTensor<uint8_t>*)myEngine::pEngine->getObject(self->tensorId);
	...
	static PyObject* fromBuffer(PyObject* self, PyObject* args)
	{
		...
		myEngine::pEngine->removeObject(s->tensorId);
		s->tensorId = myEngine::pEngine->setObject((myObject*)tensor);
		s->type[0] = p.first;
		s->type[1] = 0;
