import json
import os.path
my.Import("projects/python/graphics/model1.py")
net = CNN()
checkPointPath = my.Path("projects/python/graphics/data/model5.pth")
if(os.path.exists(checkPointPath)) == False:
	raise Exception("Please provide a valid saved model file")
...
def drawImage():
	des = np.asarray(Image.open(my.Path("projects/python/graphics/test.png")))
	print(des.shape)
